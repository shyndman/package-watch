...
- [x] 1.6 Add AliExpress order-details message type and result type
- [x] 1.7 Update `AliExpressDiscoveredOrder` to store order details URL (and remove product titles/urls)
- [x] 1.8 Add `aliexpress-order-details` parse failure phase

## 2. AliExpress Content Scripts

- [x] 2.1 Update `entrypoints/aliexpress-orders.content.ts` for order list page parsing
...
- [x] 2.3 Create `entrypoints/aliexpress-order-details.content.ts` for order details page parsing
...
- [x] 3.4 Update `performAliExpressScrape()` to three-phase flow (order list → order details → tracking)
...
- [x] 3.8 Add order details message handler + pending request tracking
- [x] 3.9 Merge order details product info into `OrderStatus` build
...
- [x] 5.3 Verify existing Amazon functionality unchanged
...
- [x] 7.1 Type-check with `pnpm compile`
- [x] 7.2 Build with `pnpm build:firefox`
- [x] 7.3 Manual test: Amazon order detection still works
- [x] 7.4 Manual test: AliExpress order list scraping
- [x] 7.5 Manual test: AliExpress tracking page scraping
- [x] 7.6 Manual test: Status change notifications fire correctly
- [x] 7.7 Manual test: Parse failure notification works
